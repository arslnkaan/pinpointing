...
#SBATCH --mem=48g
#SBATCH -t 12:00:00
#SBATCH --job-name=mismatch_CtoT_allpos
#SBATCH --output=logs/mismatch_CtoT_allpos_%A_%a.out
#SBATCH --error=logs/mismatch_CtoT_allpos_%A_%a.err
...
out="/work/users/a/r/arslank/${sample}_mismatch_pipeline_CtoT_allpos"
...
echo "Output: $out"
echo "Change: $changes"
echo "Positions from 3 prime: ${from3_min}-${from3_max}"
echo "============================================================"
...
if [[ ! -f "$dedup_bam" ]]; then
  ...
  samtools sort -@ 8 -o "$coord_sorted_bam" "$fixmate_bam"
  samtools markdup -r -@ 8 "$coord_sorted_bam" "$dedup_bam"
  samtools index "$dedup_bam"

  rm -f "$name_sorted_bam" "$fixmate_bam" "$coord_sorted_bam"

else
  echo "Dedup BAM already exists, skipping."
fi
...
echo "[2/4] Filtering C>T mismatches, all positions"
...
echo "[4/4] Plotting C>T position barplot"

event_file="${out}/02_filtered_events/${sample}_singleMismatch_C_to_T_${from3_min}to${from3_max}nt_from3prime_${min_len}to${max_len}mers.tsv"
